      ...
      <div className="relative">
        <div className="flex animate-scroll space-x-4 md:space-x-6 lg:space-x-8">
          {brandsData.concat(brandsData).map((brand) => (
            <div
              key={brand.id}
              className="flex-shrink-0 w-24 h-24 md:w-28 md:h-28 lg:w-36 lg:h-36 bg-white rounded-lg shadow-lg flex items-center justify-center p-2 transition-transform duration-500 transform hover:scale-110"
            >
              <img
                src={brand.image}
                alt={brand.name}
